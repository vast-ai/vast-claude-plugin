...
3. **Recent invoices (optional).** If the user wants a longer view, run:
   ```
   vastai show invoices-v1 -c --raw
   ```
   The `-c` flag is required and selects "charges" (use `-i` for paid invoices). Summarize `results[]` over the last three months.
