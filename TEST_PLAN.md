# Test plan — vast-claude-plugin

How to verify the plugin against a live Vast.ai account. Prompts live in `TEST_PROMPTS.txt`. This file describes setup, what to look for, and pass criteria.
...
This loads the plugin from the local checkout without installing. Inside the session, `/reload-plugins` picks up local edits without a restart.
...
The skill is the runbook. If the agent does the wrong thing, it's a skill bug — fix the relevant section in `skills/vastai/SKILL.md` (or `skills/vastai-host/SKILL.md`) and re-run the failing prompt.
